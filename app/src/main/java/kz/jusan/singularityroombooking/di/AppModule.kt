package kz.jusan.singularityroombooking.di

import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.components.SingletonComponent
import kz.jusan.singularityroombooking.common.Constants
import kz.jusan.singularityroombooking.data.remote.RoomBookingApi
import kz.jusan.singularityroombooking.data.repository.RoomBookingRepositoryImpl
import kz.jusan.singularityroombooking.domain.repository.RoomBookingRepository
import retrofit2.Retrofit
import retrofit2.converter.gson.GsonConverterFactory
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
object AppModule {

    @Provides
    @Singleton
    fun provideRoomBookingApi(): RoomBookingApi {
        return Retrofit.Builder()
            .baseUrl(Constants.BASE_URL)
            .addConverterFactory(GsonConverterFactory.create())
            .build()
            .create(RoomBookingApi::class.java)
    }

    @Provides
    @Singleton
    fun provideRoomBookingRepository(api: RoomBookingApi): RoomBookingRepository {
        return RoomBookingRepositoryImpl(api)
    }
}